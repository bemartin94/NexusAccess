...
class IdCardTypeDAL:
    ...
    async def get_id_card_type_by_id(self, id_card_type_id: int) -> Optional[IdCardType]:
        result = await self.db.execute(select(IdCardType).where(IdCardType.id == id_card_type_id)) # await añadido
        return result.scalars().first()

    async def get_id_card_type_by_name(self, name: str) -> Optional[IdCardType]:
        result = await self.db.execute(select(IdCardType).where(IdCardType.name == name)) # await añadido
        return result.scalars().first()

    async def get_id_card_types(self, skip: int = 0, limit: int = 100) -> List[IdCardType]:
        result = await self.db.execute(select(IdCardType).offset(skip).limit(limit)) # await añadido
    ...
    async def create_id_card_type(self, id_card_type_data: IdCardTypeCreate) -> IdCardType:
        ...
        await self.db.refresh(new_id_card_type) # await añadido
    ...
    async def update_id_card_type(self, id_card_type_id: int, id_card_type_data: IdCardTypeUpdate) -> Optional[IdCardType]:
        ...
        await self.db.commit() # await añadido
        await self.db.refresh(id_card_type_to_update) # await añadido
    ...
    async def delete_id_card_type(self, id_card_type_id: int) -> bool:
        ...
        if id_card_type_to_delete:
            await self.db.delete(id_card_type_to_delete) # await añadido
            await self.db.commit() # await añadido
            return True
        return False